...
- **状态**: 🔴 **待修复**
- **问题**: dagre 布局不支持有向环，互相连接的节点（如"交叉注意力" ↔ "交叉注意力回顾关注原文"）重叠放置在左上角
- **尝试方案**: 临时移除环边 → dagre 布局 → 恢复边，但用户反馈未生效
- **可能原因**: 前端代码未重新构建（`npm run build`）或浏览器缓存
- **明日排查**: 确认前端构建状态，检查 dagre 布局时边是否已正确移除

#### LA-035-P19-2: 文档树视图布局
- **状态**: 🔴 **待修复**
- **问题**: 文档树中所有节点重叠在一起，没有预期的按 heading 展开的树形结构
- **根因**: `runTreeLayout` 只处理 `type: "child"` 的节点，但当前 Chunk 类型是 `paragraph` / `heading` / `document`
- **尝试方案**: 将 `runTreeLayout` 改为 dagre TB 布局并支持所有 chunk 类型，但用户反馈未生效
- **可能原因**: 前端代码未重新构建，或 BELONGS_TO 边数据未正确加载
- **明日排查**: 确认前端构建状态，检查文档树加载的边类型是否正确
