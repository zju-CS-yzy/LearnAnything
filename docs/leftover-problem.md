...
- **LA-053**: 概念树布局重叠/凌乱
  - **状态**: ✅ **已解决**
  - **修复**:
    1. 小树（≤5节点）改用 BFS 层次树形布局（替代 dagre）
    2. 树间间距增大（treeGapX: 60→120, treeGapY: 80→150）
  - **验证**: 用户确认树形结构清晰，无重叠
  - **优先级**: P1 → 已关闭
...
- **LA-057**: 双击节点时图谱布局失效
  - **状态**: ✅ **已解决**
  - **根因**: `expandNode()` 在概念视图下错误调用 `runTreeLayout()`（文档树布局）
  - **修复**: `GraphView.vue` 根据 `viewMode` 调用正确布局函数
  - **验证**: 双击后布局正常
  - **优先级**: P1 → 已关闭

- **LA-058**: 小树（2-3节点）布局失效
  - **状态**: ✅ **已解决**
  - **根因**: 小树用简单网格排列，未考虑父子关系
  - **修复**: BFS 层次布局（根→子→孙，水平展开）
  - **验证**: 用户确认小树形成清晰的树形结构
  - **优先级**: P1 → 已关闭
...
- **LA-044-#2**: TutorAgent 个性化 Prompt（根据 user_theta 调整讲解深度）
  - **状态**: ✅ **已实现并测试通过（2026-07-24）**
  - **实现内容**:
    1. `TutorAgent.__init__` 新增 `user_theta` 参数
    2. `_generate_answer` 根据 theta 区间注入个性化 system_prompt 后缀
       - 初学者 (0.0-0.3): 类比、避免跳步、详细解释
       - 中级 (0.31-0.7): 平衡、适当跳步、知识网络
       - 高级 (0.71-1.0): 聚焦核心、大量跳步、技术细节
    3. `Coordinator` 和 `AskRequest` 均支持 `user_theta` 传递
  - **测试**: `scripts/test_la044_theta_personalization.py` 10 用例全部通过
  - **优先级**: P1 → 已关闭
...
- **LA-052**: 范式配置动态获取（前端）
  - **状态**: 🟡 **后端已完成，前端待实现**
  - **说明**: 后端 API / ParadigmManager / ParadigmValidator 全部完成
  - **待完成（需用户在场测试）**:
