...
VLMClient (GLM-4V) → 图片描述 → 注入 chunk.text
    │
    ▼
VectorStore (ChromaDB) + GraphStore (KuzuDB) + SubjectManager (SQLite)
    │
    ▼
...
SemanticLinker → 构建概念间连接（SOLUTION/DEPENDS_ON）
    │
    ▼
GraphStore → 写入 KuzuDB（CanonicalConcept + 语义边）
```
...
| **图数据库** | KuzuDB | Neo4j, Dgraph, ArangoDB | KuzuDB 是嵌入式图数据库，单文件存储，Cypher 兼容，无需额外服务；Neo4j 需要独立服务器 |
...
#### 3.1.3 创新机制：语义分解、范式连接与 parent_hint

本小节介绍 LearnAnything 在知识图谱构建中的三个核心创新机制：语义分解与投影思想、基于范式的语义连接、以及 parent_hint 启发式线索。

##### ① 语义分解与投影思想

LearnAnything 构建知识图谱的目标是构建RAG中的chunk之间的联系，而构建这种联系的思路的根本出发点则是一个认识论问题：**人类如何认识世界？**

我们的核心思想是：

> **意义的本质是概念空间中的向量。** 例如「黑色的球」所描述的意义就是由「黑色」这个「颜色」维向量和「球」这个「形状」维向量相加组成的。将意义通过向量化分解为概念的组合后，我们可以立刻建立"联系"的概念：若两个意义向量存在相同的概念维分向量，则可称这两个意义向量之间存在直接联系。例如「黑色的球」和「白色的球」就以「球」这个相同的形状维分向量建立了直接联系。同时，假设两个意义向量 A 和 B 不存在直接联系，但是存在另外一个意义向量 C 能够与这两个意义向量同时建立直接联系，则称这两个意义向量 A 和 B 之间存在间接联系，例如「黑色」和「球」之间并不存在直接联系，但是可以通过「黑色的球」来建立间接联系。显然的，概念空间中的一切向量都存在间接联系。这种意义的向量化和联系的定义为我们提供了找出任意两个意义向量之间的联系的工作范式：我们只需要以合适的方式分解这两个意义向量，使得其分解后在某个子意义方向具有相同的值，就可以建立这两个意义向量之间的联系。

基于上面的意义的向量化与联系建立的思想，我们要建立知识库中 chunk 的联系的关键目标就变成了：

1. **每个 chunk 都代表一个意义向量，需要以合适的方式将 chunk 分解为子意义向量**
2. **通过子意义向量之间的关系建立多个意义向量之间的联系网络**

此处的问题在于：这种将 chunk 的意义向量分解为子意义向量的"合适的方式"是什么？

我们所参考的结构，来自于《技术的本质》一书，该书的观点认为：**一切技术都是由可分解的子技术组成的，分解到最后则是直接应用物理现象的"原理"。** 要设计一个新技术，首先需要分析需求，将需求转化为新技术，再从已有的技术库和原理库中选择合适的组合，形成所要开发的新技术所对应的子技术。这种结构与我们分解子意义向量的思想非常相似。例如，我们要学习 RAG 技术，则构建联系的方向为：

- **上层需求**：为了解决什么问题而需要这种技术
- **子需求分解**：要构建这一技术，会产生什么子需求
- **子需求解决**：要满足子需求，需要开发什么子技术

这种递归的树状结构使得每个技术描述的 chunk 被分解为向上的需求方向 concept 和若干个向下的子需求concept，每个需求描述 concept又被分解为向上的技术方向 concept和若干个向下的子技术 concept。**这种分解方式反映了每个 concept在知识库整体结构中的位置，并且最终形成分形且递归的树形结构。**

##### ② 从 Chunk 到 ExtractedConcept：基于 LLM + 范式的概念分解

**①** 节阐述了语义分解的认识论原理和理论来源，本节说明其在代码中的具体实现——**如何将一个文本 chunk 通过 LLM 和范式模板分解为结构化的 ExtractedConcept**。

**输入**：
- 单个 chunk 的文本内容（已截断到 4000 字符）
- 可选的 heading_context（同一 heading 下的上下文，截断到 300 字符）
- 用户选择的范式（theory/engineering/hierarchical）
...
标记隔离：【知识片段开始】...【知识片段结束】
    │
    ▼
构建 system_prompt（范式模板）
    ├─ 角色定义（concept_type: requirement/technology/sub_requirement/sub_technology）
    ├─ 关系定义（relation: REQUIRES/IMPLEMENTS/HAS_SUB/HAS_IMPL）
    ├─ 判断标准（严格遵循的提取规则）
    └─ parent_hint 填写规则（用于构建层级连接）
    │
    ▼
LLM 调用（DeepSeek API, temperature=0.1, max_tokens=1200）
    │
    ▼
JSON 解析 → concept 列表
    │
...
验证与过滤
    ├─ 过滤空名称
    ├─ 过滤模糊概念（_is_vague_concept：如"完整聚合""微调嵌入"）
    ├─ 校验 concept_type 是否在范式定义的合法类型中
    └─ 校验 relation 是否在范式定义的合法关系中
    │
    ▼
输出：ExtractedConcept 列表
    [
      {
        "name": "多GPU并行训练",
        "concept_type": "technology",
        "relation": "IMPLEMENTS",
        "description": "采用多GPU并行训练以提升模型训练效率",
        "parent_hint": "提升训练效率",
...
1. **标记隔离（LA-035-P10）**：使用 `【知识片段开始】...【知识片段结束】` 明确隔离用户内容与系统指令，避免 LLM 从系统提示中误提取概念

2. **上下文声明（LA-035-P12）**：heading_context 作为 `【上下文声明】` 注入，帮助 LLM 理解语义位置，但明确禁止从中提取概念

3. **批量提取优化（v2）**：
   - 按 heading 分组，同一 heading 内的 chunk 一起提取
   - 系统提示词只发一次，节省 60-80% token
   - 每个 chunk 最多 800 字符，总批次不超过 3000 token
   - 优势：同一 heading 的 chunk 上下文连贯，LLM 理解更准确；减少 API 调用次数，整体提速 3-5x

4. **模糊概念过滤**：`_is_vague_concept` 方法过滤掉没有上下文就无法理解的概念（如只有 2-4 个字且以"聚合/嵌入/优化"等模糊后缀结尾的概念），避免噪声污染图谱

**与 SemanticLinker 的工作流程关系**：

```
Phase 2: 概念分解（SemanticExtractor）
    │
    ▼
ExtractedConcept 列表（含 name, concept_type, relation, parent_hint）
    │
    ▼
Phase 2.5: 概念去重（ConceptDeduper）
    │
    ├─ 基于 embedding 相似度 > 0.85 的贪心合并
    ├─ 合并后生成 CanonicalConcept
    └─ 建立 ExtractedConcept → CanonicalConcept 的映射
    │
    ▼
Phase 2.5: 语义连接（SemanticLinker）
    │
    ├─ _stage1_parent_hint_match:
    │   parent_hint 字符串 → 上层概念 canonical name 精确匹配
    │   → 按 PARADIGM_LEVELS transitions 映射关系类型
    │
    ├─ _stage2_embedding_similarity:
    │   embedding 相似度 → 补充跨章节的语义连接
    │
    └─ _stage3_llm_confirm:
    │   LLM 二次确认连接合理性
    │
    ▼
CanonicalConcept 树（含 SOLUTION/DEPENDS_ON 语义边）
```

**核心洞察**：SemanticExtractor 的分解和 SemanticLinker 的连接是**两阶段分离**的设计——先让 LLM 在每个 chunk 的局部上下文中提取概念和 parent_hint，再在全局视角下（通过字符串匹配 + embedding + LLM 确认）建立连接。这种分离使得：
- 局部提取不受全局知识影响（避免 LLM 过度推断）
- 全局连接可以利用跨 chunk 的上下文（通过 embedding 和 LLM 确认发现隐性关联）
- parent_hint 作为"桥梁"，将 LLM 在局部提取时的语义判断传递给全局连接阶段
...
**范式有效性（以[工程分解]范式为例）：来自《技术的本质》的理论基础**
...
##### ③ parent_hint 机制：文档结构到概念结构的桥梁

parent_hint 是 LearnAnything 中用于**连接文档层级结构（Phase 1）和概念层级结构（Phase 2）**的启发式线索。需要明确区分两种不同场景：

| 场景 | 作用域 | 连接类型 | 生成方式 | 示例 |
|:---|:---|:---|:---|:---|
| **文档树 BELONGS_TO** | Chunk 层 | BELONGS_TO | MarkdownChunker 基于 heading_path 结构生成 | `RAG概述.md` → `RAG关键痛点.md` → `痛点1.md` |
| **概念树 parent_hint** | CanonicalConcept 层 | SOLUTION/DEPENDS_ON | SemanticLinker 基于范式层级匹配 + LLM 确认 | `RAG` -(SOLUTION)→ `向量检索` |

**文档树 parent_hint 的实现原理**：

文档树的层级关系由 **MarkdownChunker v2.0** 在分块阶段生成，而非 Phase 2 的概念提取阶段。其核心机制是 `heading_path` 字段：

1. **分块阶段**：MarkdownChunker 解析 Markdown 的标题层级（`#`, `##`, `###`），为每个 chunk 生成 `heading_path` 字段。例如：
   - `RAG概述` 标题的 chunk：`heading_path = "RAG概述"`
   - `RAG关键痛点` 子标题的 chunk：`heading_path = "RAG概述 > RAG关键痛点"`
   - 该子标题下的段落 chunk：`heading_path = "RAG概述 > RAG关键痛点"`

2. **关系建立**：`build_belongs_to_relations()` 遍历所有 chunk，通过 `heading_path` 的**前缀匹配**建立 BELONGS_TO 关系：
   - 找到所有 heading 节点，建立 heading 之间的层级关系（通过去掉最后一级的 heading_path 找父 heading）
   - 对于每个 paragraph/image 节点，找到其 heading_path 对应的 heading 节点，建立 BELONGS_TO 关系
   - 对于没有 heading_path 的 paragraph（如文档开头的前两段），直接建立到 document 节点的 BELONGS_TO 关系（LA-035-P35 修复）

3. **与概念树的区别**：文档树的 parent_hint 是**结构化的、确定性的**——它完全由 Markdown 的标题层级决定，不依赖 LLM 的语义理解。这与概念树的 parent_hint（基于 LLM 提取的语义角色和字符串匹配）有本质区别。

**概念树 parent_hint 的实现原理**：

1. **提取阶段**：SemanticExtractor 在提取概念时，为每个 concept 标记 `concept_role`（如 requirement/technology）和 `parent_hint`（指向父概念的名称）
2. **连接阶段**：SemanticLinker 的 `_stage1_parent_hint_match` 将 `parent_hint` 与上层概念的 canonical name 进行**精确字符串匹配**
3. **关系映射**：匹配成功后，根据 `PARADIGM_LEVELS` 预定义的 transitions 映射关系类型（如 requirement→technology = SOLUTION）
...
**未来方向：基于语义角色的智能 parent_hint 推断**

当前 parent_hint 的实现基于**字符串精确匹配**（`parent_hint` 字段值与上层概念名称匹配）。更智能的方案是：

> 当 LLM 提取一个 concept 并判断其角色为 "technology" 时，自动分析该 concept 是为了解决什么 "requirement"，然后在所有 requirement 类型的 concept 中语义匹配，建立 parent_hint。

这种**基于语义角色的反向推断**将完全消除对字符串匹配的依赖，但目前尚未在代码中实现。

#### 3.1.4 同类项目定位与优劣势

| 项目 | 架构 | 节点类型 | 层级 | 概念去重 | 最接近的层 |
|:---|:---|:---|:---|:---|:---|
...
3. 图数据库 KuzuDB 的社区生态较 Neo4j 弱，高级功能（如社区检测、图算法）需要自行实现

### 3.2 知识图谱可视化模块

LearnAnything 提供两种可视化视图，服务于不同的学习目标：

| 视图 | 数据层 | 结构类型 | 核心用途 | 边类型 |
|:---|:---|:---|:---|:---|
| **文档树** | Chunk 层（document/heading/paragraph） | 树形结构（严格层级） | **辅助查询**：帮助用户快速定位原文位置，验证概念来源 | BELONGS_TO（层级归属） |
| **概念图谱** | CanonicalConcept 层 | DAG（有向无环图，允许共享子节点） | **核心学习**：展示概念间的语义关系，支持知识推理和路径追溯 | SOLUTION、DEPENDS_ON、PART_OF、REQUIRES |

两种视图的关系：**文档树是概念图谱的"底座"和"验证器"**——每个概念节点都可以通过 `source_chunks` 字段回溯到文档树中的具体 chunk，实现"概念→原文"的精确溯源。

---

#### 3.2.1 文档树可视化（辅助查询工具）

**定位**：文档树是**辅助性视图**，帮助用户在概念图谱中发现某个概念后，快速定位其在原始文档中的位置，验证 LLM 生成的题目/讲解是否基于真实材料。

**数据加载流程**：

```
用户切换学科 → 触发 loadAllNodes(viewMode='document')
    │
    ▼
loadChunkNodes() → 后端 API /api/knowledge-graph/{subject}/nodes
    │
    ▼
后端 GraphStore → KùzuDB 查询 Chunk 节点（document/heading/paragraph/image）
    │
    ▼
...
3. 计算每棵树的 bbox，从左到右排列（树间距 150px），避免多棵树重叠
4. 孤立节点放最右侧网格
5. 调整边曲率（adjustEdgeCurvature）：子节点在上→向上凸，在下→向下凸，同级→直线
6. 手动 fit（zoom = min(容器高/图高, 容器宽/图宽, 0.5)，限制 0.15-0.5）

**节点风格**：

- document 节点：绿色卡片，显示文档名称
- heading 节点：红色卡片，显示标题
- paragraph 节点：蓝色卡片，显示段落摘要（前 22 字符）
- image 节点：图片预览（background-image）或 📷 回退

---

#### 3.2.2 概念图谱可视化（核心知识图谱）

**定位**：概念图谱是**核心学习视图**，展示 CanonicalConcept 之间的语义关系（SOLUTION、DEPENDS_ON 等），是 Quiz Agent 出题、Coach Agent 测评、Tutor Agent 讲解的**直接数据源**。

**数据加载流程**：

```
用户切换学科 → 触发 loadAllNodes(viewMode='concept')
    │
    ▼
loadConceptNodes() → 后端 API /api/knowledge-graph/{subject}/concepts
    │
    ▼
后端 GraphStore → KùzuDB 查询 CanonicalConcept 节点
    │
    ▼
前端生成 UML 卡片数据（cardLabel, cardHeight, nodeWidth, borderColor）
    │
    ▼
cy.add(conceptNodes) → Cytoscape.js 渲染
    │
    ▼
loadSemanticEdges() → 后端 API /api/knowledge-graph/{subject}/semantic-edges
    │
    ▼
前端加载 SOLUTION/DEPENDS_ON/PART_OF/REQUIRES 边 → cy.add(edges)
    │
    ▼
runConceptLayout() → DAG 布局（副本处理 + dagre LR + 固定 zoom）
```

**布局算法（runConceptLayout）**：

概念图谱的结构是 **DAG（有向无环图）**，允许一个概念被多个父概念引用（共享子节点），这与文档树的严格树形结构有本质区别。布局算法需要处理以下挑战：

**挑战 1：DAG 的多入边问题**

在 DAG 中，一个节点可以有多个入边（如"向量检索"既被"RAG"依赖，也被"语义搜索"依赖）。标准 dagre 布局不支持多入边，会导致布局冲突。

**解决方案：副本处理（Copy Strategy）**

1. 识别所有入度 > 1 的节点（共享子节点）
2. 保留第一个入边的连接，对后续入边创建节点的**副本**（copy node）
3. 副本节点继承原节点的所有数据（label、cardLabel、description、source_chunks 等），但添加 `isCopy: '1'` 和 `originalId` 标记
4. 副本边标记为 `isCopyEdge: '1'`，样式使用虚线
5. 原节点和所有副本在视觉上保持一致，用户点击任意副本都会打开同一个概念的详情面板

```
原始 DAG 结构:          副本处理后:
  RAG ──┐                 RAG ──→ 向量检索
        ↓                          （实线，原始节点）
  语义搜索 ─┬→ 向量检索      语义搜索 ──→ 向量检索_copy1
            │                             （虚线，副本）
            ▼
         嵌入模型
```
...
概念图谱可能包含多个不连通的子图（如不同学科的概念没有连接）。需要：

1. 使用无向图 BFS 识别所有连通分量
2. 对每个连通分量独立跑 dagre LR 布局
3. 连通分量之间从左到右排列（间距 150px）
4. 孤立节点（无入边也无出边）单独网格排列

**挑战 3：固定 zoom 避免过度缩放**

dagre 的 `fit: true` 会导致大图过度缩小、小图过度放大。解决方案：

```javascript
zoom = Math.min(容器高度 / 图高度, 容器宽度 / 图宽度, 0.5)
zoom = Math.max(zoom, 0.15)  // 最小 zoom 0.15
```

这样既保证图完整可见，又不会过度缩小导致文字不可读。

**节点风格**：

- UML 卡片风格：自适应宽高，显示概念名称、类型标签、描述摘要
- 颜色编码：不同 borderColor 区分概念类型（绿色=核心概念，蓝色=技术，红色=需求）
- 媒体标记：右下角显示 📷/📊/📝 标记，指示该概念关联的媒体类型
- 副本标记：虚线边框，与原始节点区分
...
| **布局算法** | dagre（LR 方向） + 手动排列 | COSE, Grid, Circle, Cola | dagre 层次清晰，适合树形/DAG 数据；COSE 适合力导向网络图 |
...
|:---|:---|:---|:---|:---|
| **GraphRAG (Microsoft)** | 无原生可视化 | 无 | 无 | 仅提供后端 API，无前端可视化；本项目自研双视图可视化 |
| **LightRAG** | 无原生可视化 | 无 | 无 | 同上 |
| **Neo4j Browser** | 内置可视化 | 力导向 | 圆形标签 | 通用图数据库浏览器，不针对学习场景；不支持 DAG 副本处理 |
| **Obsidian (Graph View)** | 本地笔记图谱 | 力导向 | 文本标签 | 面向笔记关联，节点是笔记文件；本项目节点是概念/段落，粒度更细 |
| **LearnAnything (本项目)** | 自研 Vue + Cytoscape | dagre LR + 副本处理 + 手动排列 | UML 卡片（自适应宽高） | **文档树+概念图谱双视图**，文档树支持原文溯源，概念图谱支持 DAG 副本处理 |

**核心优势**：
1. **双视图协同**：文档树（原文定位）+ 概念图谱（语义学习）互相补充，概念节点可一键跳转到文档树中的来源 chunk
2. **DAG 副本处理**：解决多入边节点的布局冲突，保持图谱可读性，同时保留完整的语义关系
3. **卡片节点**：自适应宽高的 UML 卡片风格，显示节点类型、标题、原文摘要，信息密度高
4. **悬浮预览**：鼠标悬停显示原文摘要、图片缩略图、媒体标记，无需点击即可快速浏览
5. **详情面板**：点击节点显示完整原文内容、来源 chunks、章节路径，支持精确溯源

**劣势**：
1. 布局算法复杂（连通分量识别 + 副本处理 + dagre + 手动排列），实现成本高
2. 段落排序尚未实现（dagre 内部排序不可控），同级节点顺序依赖原始数据
3. 大规模图（>1000 节点）性能需要优化（当前渲染 441 节点流畅，但 1000+ 可能有卡顿）
4. 副本节点在视觉上与原始节点相似，用户可能难以区分"同一概念的不同引用"
