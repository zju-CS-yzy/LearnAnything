...
LearnAnything 构建知识图谱的根本出发点是一个认识论问题：**人类如何认识世界？**
...
这种递归的树状结构使得每个技术描述的 chunk 被分解为向上的需求方向 chunk 和若干个向下的子需求 chunk，每个需求描述 chunk 又被分解为向上的技术方向 chunk 和若干个向下的子技术 chunk。**这种分解方式反映了每个 chunk 在知识库整体结构中的位置，并且最终形成分形且递归的树形结构。**
