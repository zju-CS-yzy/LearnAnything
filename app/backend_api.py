...
def get_config():
    ...
    return ConfigResponse(
        llm=_to_item(cfg.llm),
        llm_fallback=_to_item(cfg.llm_fallback),
...
class ConfigUpdateRequest(BaseModel):
    """配置更新请求"""
    llm: Optional[LLMConfigItem] = None
    llm_fallback: Optional[LLMConfigItem] = None
...
def update_config(request: ConfigUpdateRequest):
    ...
    def _merge_item(current: FeatureConfig, incoming: Optional[LLMConfigItem]) -> FeatureConfig:
        ...
        return FeatureConfig(
            ...
            base_url=incoming.base_url if incoming.base_url else current.base_url,
            model=incoming.model if incoming.model else current.model,
